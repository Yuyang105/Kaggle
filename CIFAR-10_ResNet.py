...
class Residual(nn.HybridBlock):
    # 参数： channel数，same_shape:因为要做加法，所以shape一不一致很重要
    def __init__(self, channels, same_shape=True, **kwargs):
        ...
        with self.name_scope():
            ...
            # 我们需要把shape变成一致，不是same_shape，我们shape被减半了
            if not same_shape:
    ...
    def hybrid_forward(self, F, x):
        ...
        # 如果不是same_shape，x做第三层卷积，放回out的形状
        if not self.same_shape:
            x = self.conv3(x)
        return F.relu(out + x)

# ResNet就是n个Residual块相连    
